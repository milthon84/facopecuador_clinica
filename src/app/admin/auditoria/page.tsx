...
export default async function AuditoriaPage({
  ...
  if (sessionRole !== "admin") redirect("/admin");
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <form className="flex flex-wrap gap-3 mb-6" method="get">
        ...
        {(searchParams.resource || searchParams.action) && (
          <a href="/admin/auditoria" className="bg-white border border-lilac-200 hover:bg-lilac-50 text-ink-700 px-4 py-2 rounded-xl text-sm font-medium transition-colors">
            Limpiar filtros
          </a>
        )}
      </form>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-lilac-50 text-sm">
            ...
            <div className="flex gap-2">
              {page > 1 && (
                <a
                  href={`/admin/auditoria?page=${page - 1}${searchParams.resource ? `&resource=${searchParams.resource}` : ""}${searchParams.action ? `&action=${searchParams.action}` : ""}`}
